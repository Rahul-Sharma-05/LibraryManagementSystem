...
<body>
	...
	try {
		...
		if (btn.equalsIgnoreCase("Submit Form") || btn.equalsIgnoreCase("Add Student")) {
			...
			"INSERT INTO Student (StudentName, StudentEmail, Password, StudentContact, StudentCity, Gender) VALUE (?,?,?,?,?,?)");
	...
	<%
		...
		if (btn.equalsIgnoreCase("Submit Form")) {
			out.println(
					"<script>alert('Registration Completed. Your Student Id has been sent to your emailaddress.')</script>");
	%>
	<script>
	...
	%>
	<script>
		window.location.href = "Logout?btn=slogout";
	</script>
